...
def assigne_avaprobs( df_P, thresholds):

    # thresholds
    inithrshnap =  thresholds['inithrshnap']    # do not constrain
    propthrshnap = thresholds['propthrshnap'] 
    damthrshnap =  thresholds['damthrshnap']  # snp: .32m
    damthrshpap =  thresholds['damthrshpap']      # snp:18 8 18 hours
    # inithrshdap= 2x inithrshpap; propthrshdap= 2x propthrshpap;
    inithrshpap = thresholds['inithrshpap']       # snp:1;
    propthrshpap = thresholds['propthrshpap']     # snp:.24 .318 .4;
    drftthrsh = thresholds['drftthrsh']         # snp.5[m]; min height of snow drifts
    dysisomax =  thresholds['dysisomax']
    lwcthrsh  = thresholds['lwcthrsh']

    lwcthrsh_0 = thresholds['lwcthrsh_0'] 
    lwcthrsh_1 = thresholds['lwcthrsh_1'] 
    release = thresholds['release']
    # Postprocessing the find aps out--> give avaproblems
    
    # look at all daps of 1 season
    ...
    for index,row in df_P.iterrows(): # %days with wind loading
        print(df_P['dapex'][index])

        if ~np.isnan(df_P['dapex'][index]) :
            nr_daps = len(df_P['dapINI_tau_p'][index]) # nr of daps of specific day
    ...
    #if release == 'natural': # NATURAL RELEASE
    ...
    #elif release == 'trigger': # %ARTIFICIAL TRIGGERING
    ...
    papno = np.sum(df_P['papex_sele_trigger'])
    print('nr of paps trigger:',papno)
    print('nr of paps natural:',np.sum(df_P['papex_sele_natural']))

 
    #%% look at all naps of 1 season
    ...
    #        print(np.sum(df_P['napex_sele'] == 1))
     #       print(np.sum((df_P['napINI_isrel'] <= inithrshnap) & (df_P['napPRO_isrel'] <= propthrshnap)))
      #      print(np.sum(df_P['napdycrit']))
    napno = np.sum(df_P['napex_sele_trigger'])
    print('nr of naps trigger:' ,napno)
    print('nr of naps natural:' ,np.sum(df_P['napex_sele_natural']))

    #%% WE ALSO NEED TO GET THE WIND INDICATOR FOR WIND LOADING (rather than new snow loading -- pure wind slab case)
    #% sele=find(P.napwindonly);   %days with neither pap nor nap, but wind loading
    df_P['winex'] = np.NaN
    #df_P['winex'] = df_P['napwindonly']  # %days with neither pap nor nap, but wind loading
    df_P['winex'] = np.where ( (df_P['drft']> drftthrsh ) , 1, 0 )  

    # # %% DAP is secondary release of new snow, wind slab AND/OR persistent?
    #if release == 'natural':
    df_P['dapex_sele_natural'] = np.where((df_P['dapex_isrel_red']== 1) & (~ (df_P['napdycrit_natural'] == 1)),  0, df_P['dapex_isrel_red'])
    #elif release == 'trigger':
    ...
    dapno = np.sum(df_P['dapex_sele_trigger'])
    print('nr of daps trigger', dapno)
    print('nr of daps natural', np.sum(df_P['dapex_sele_natural']))
    
    #%% look at all waps of 1 season
    ...
    wapno= len(df_P['wapLWC_isrel'][df_P['wapLWC_isrel']> 0])
    print('nr of waps:',wapno)
