...
# find avalanche proplem types aps
...
import fu_tau_p_CoJ15 as fu_tau_p_CoJ15
import fu_instab as pro_instab
from snowpro import snowpro
#import snowpro as pro_pro
#import processor_PRO as pro_pro  # function to read in .pro files

# import read_smet_file as pro_smet

def find_aps(season_start, season_end, drytime,wettime,thresholds, pro_path, smet_path,  read_in = 1, initilization_type='station'):

    # find avalanche problem types (datetime-s)
    #pap: persistent (avalanche problem type), keep, becomes dap if more recent pap appears
    #nap: non-persistent (avalanche problem type), keep for +1day unless HN24 exceeds threshold
    #dap_yymmdd: deep (persistent) problem, was pap before, has label of WL burial date
    #-----------------------------------------------------------------------

    ## matlab line 40
    alp = thresholds['alp']
    calcFEM = thresholds['calcFEM']                     #%=1; means use FEM model to calcuate shear stress at WL, slab modulus, slab tensile stresses
    owSCtaup = thresholds['owSCtaup']                    #%=1; means overwrite snow cover shear strength (tau_p)
    lookat = thresholds['lookat']                      #%say if you want to see a graph
    aggrgtDAPs = thresholds['aggrgtDAPs']; aggrgtdiff = thresholds['aggrgtdiff'] #%aggregate DAPs if they are located within aggrgtdiff [m]
    outputDAP = thresholds['outputDAP']                   #% index of DAP to output; can create table: table(DAPLAYEROUT_txt,DAPLAYEROUT_dou)
    #%% basic decision data   ## matlab line 84
    dropini = thresholds['dropini']
    droppro = thresholds['droppro']            #thresholds of initiation and propgation criteria for dropping (deep-)persitent weak layers
    drftthrsh = thresholds['drftthrsh']          #min height of snow drifts [m], snp=:0.4m, cro:.2

    minSLdens = thresholds['minSLdens']         #120,min average density of a potential slab
    minnsthrsh = thresholds['minnsthrsh']        #def.: 0.05m,min. amount of new snow [m], to start identifying problems (we find WL with windows, do don't care about +-5cm of fresh snow)
    nsthrsh = thresholds['nsthrsh']            #def.: .2,amount of new snow [m] (within 48 or 24 hr), to keep a non-persis. problem for one more day
    minSLthrsh = thresholds['minSLthrsh']        #def.: .2,min. slab thickness [m] to be considered a healthy slab, 
    lwcthrsh = thresholds['lwcthrsh']        #.008[snp], Christophs LWC threshold for maztrix flow: 0.03; threshold for preferential flow: 0.01
    
    lwcthrsh_0 = thresholds['lwcthrsh_0'] 
    lwcthrsh_1 = thresholds['lwcthrsh_1'] 
    # minfracswetsnow=0.7   #min fraction of snow height for wap, def.: 0.7 * max snow height during season

    if read_in == 1:
        # - read .pro-file (list of df containing single profiles)
        print('...generate list of profiles list_df_pro')
        list_df_pro, meta_dict = snowpro.read_pro(pro_path)
        print('Slope Angle: ', meta_dict['SlopeAngle'])
        print('Aspect: ', meta_dict['SlopeAzi'])

        print('...generate df_met')
        df_met = snowpro.get_smet_df(smet_path)

        dump_profs_and_smet=0
        ...
        print('...read in existing list_df_pro and df_met')
    ...
    else: # if ini == digged profile data starts with observation
        df_met_red=df_met
    #df_met = df_met.loc[mask_season].reset_index(drop=True)
    #reduce metfiles to dry&wet time
    ...
    #create empyt list for dry and wet time
    season_list = []
    season_list_red = []
    drytime_list = []
    wettime_list = []
    ...
    if initilization_type=='station':
        #reduce dataset to season start/end
    ...
    #reduce season_list to dry&wettime:
    ...
    # file structure for avalanche problems
    P_varibales = [
    ...
    # gernerate empty df_P
    # just take days into account (YYYY:MM:DD not YYYY:MM:DD HH:MM:SS )
    ...
    if initilization_type=='station':
        ...
        df_P.loc[1:,'dy'] = date.values   #add timestamp from metfile to final df
    ...
    cycle_nr = 1 # traking of wet cycles
    runno =1
    print('...checking for weaklayers')
    ...
    if initilization_type=='profile':
        ...
        if i_layer>0:
            ...
            if sum(id_p.values) > 0:
                ...
                print( 'Initialization of pap... ', df_P.loc[0,'papgt'],'down',df_P.loc[0,'papSLdep']*100,'cm')
            else:
                # - NAP found - #
                rho = 'density'
                h = 'thickness_m' 
                ...
                #    % slab may ex. --> calculate INI and PROP
                    print('    slab may ex. --> calc instab indices')
                    ...
                    print(' no healthy slab --> keep nap for +1 day')
                ...
                print('Initialization of nap... ')

            i_old_surf = i_layer
        else:
            print('No nap or pap from initial snow profile')
            i_old_surf = 0
            pap_range_below_surf = 0

        ### - Initialize DAP - ###
        print('Initialization of dap... ')
        DAP_i_layer = []
        DAP_up_ini  = []
        DAP_gt_ini  = []
        ...
        if df_P.loc[0, 'papex'] != 1:
            ...
            print( 'Initialization of pap from uppermost dap... ', df_P.loc[0,'papgt'],'down',df_P.loc[0,'papSLdep']*100,'cm')
    ...
    for index in range(1,len(df_P)):
        ...
        dy = df_met_red.timestamp.iloc[ind_dry] # date
        # adress snowheight from smet to final df acording to date
        ...
        #read in profile from specific day  
        ...
        po= 'height_m' #height [> 0: top, < 0: bottom of elem.] (m)'# just for better readability
        if  df_prof[po][0]> 2 * nsthrsh: # if not df_prof.empty relict matlab... or else there is no snow (hs<.4m), so skip the procedure      
    ...
    # #########################################################################
    # ##### #%% %% %% %% %% %%   FIRST find prior problems  %% %% %% %% %% %%matlab line 143
    # #########################################################################
        #%i.e. preexisting from the day before
            
            #if index == 0: #catching the entrance problem with index0
             #   pass
             # find prior persistent problem (pap) in current layfile matlab 146
            if df_P.loc[index-1, 'papex'] == 1:
                print('prior exiting WL', index)    
                ...
                Lupnew,Llonew,Lgtnew,Lrho,SLdep,SLrho,layfound = find_lay_prof(df_prof, df_P['papup'][index-1],df_P['papgt'][index-1], 'per')
                ...
                #% treat problem, if persis. WL layer found
                    ...
                    if (df_P['papSLdep'][index]> minSLthrsh and df_P['papSLrho'][index]>minSLdens) or df_P['drft'][index]>drftthrsh:
                         #% slab may ex. --> calc. snow instability
                        print('     slab may ex. --> calc instab indices')
                        ...
                        #    print( ' INI:S=',(df_P['papINI_tau_p'][index] / df_P['papINI_mssANA'][index]*10)/10,' PROP:ac=',(df_P['papPRO_ac_vh16'][index]*100),'cm / Sk_',alp,'=',(df_P['papINI_Sk'][index]*10)/10)
                         ...
                         #% no slab --> but keep label as persistent problem for future
                        print('    no slab --> keep pap')
                        df_P.loc[index, 'papcalc']  = 0 
                        #% shear strength keeps evolving matlab 174
                        # fu tau makes it slow
                    ...
                    print('    no WL found --> drop pap')
                    df_P.loc[index,'papex']  = 2
                    df_P.loc[index,'papcalc'] = 0           
    
             # %%% find prior non-persistent problem (nap) in current layfile -matlab 184
            
            if df_P.loc[index -1,'napex']  == 1:
                df_P.loc[index,'napburial'] = df_P.loc[index - 1,'napburial'] 
                Lupnew,Llonew,Lgtnew,Lrho,SLdep,SLrho,layfound = find_lay_prof(df_prof, df_P.loc[index-1, 'napup'],df_P.loc[index-1,'napgt'], 'nonper')
                ...
                if layfound >0:
                    ...
                    if df_P.loc[index,'hn24' ] > nsthrsh or \
                        ...
                        print('    more snow added --> calc instab indices') #- matlab line193
                        df_P.loc[index, 'napcalc']= 1
             ...
             ##%%% find older deep persistent problems (dap) and treat ALL of them
             #%% ! check DAP relevance (SLdep, SLrho); (a dap could develop from an unrelevant pap)
            if df_P.loc[index - 1,'dapex'] == 1:
                
                df_P.loc[index,'dapex'] = 1 # % per def.: yesterday's problems are today's problems:
                ...
                for ele in range(len(df_P['dapup'][index])):
                    ...
                    if ~np.isnan( df_P.loc[index-1, 'dapup'][ele]) and  df_P.loc[index-1, 'dapup'][ele] > 0.1 : #no processing of dropped daps or daps settling towards the ground
                        
                        #if df_P['dapgt'][index-1][ele][0] == 'FCxr':  ## for matlb comparison (matlab loses FCxr)
                         #   df_P['dapgt'][index-1][ele][0] = 'FC'
                         
                        Lupnew,Llonew,Lgtnew,Lrho,SLdep,SLrho,layfound = find_lay_prof(df_prof, df_P.loc[index-1,'dapup'][ele],df_P.loc[index-1,'dapgt'][ele], 'per') 
                        ...
                        # loesch = df_P.loc[index,'dapburial'][ele]
                ...
                if outputDAP !=0:
                    print('implement line 254')

                #% update layer positions and properties of all daps:
    # ######## implement matlab line 214-241-256
    #             # for .....
            
    #########################################################################
    ####   %% %% %% %% %% %%  SECOND, identify new problems  %% %% %% %% %% %%  Matlabcode 265
    ########################################################################
            # %% check snow cover wetness
            #    % North and south facing slopes, need isothermal days (all
            #    % LWC_layers>.1!), after day 5 return to stability, wait until LWC_ind
            #    % INCREASES again and isothermal
            #    if water in snow cover, read wettime profile (~16:00:00) to get the snow covers's average lwc!            
            # if index % 2 == 1: #even index equals 15:00
            #      indwet = index
            #else: # odd index equals 06:00 therefore add to go to 15:00
            #     indwet = index +1
            if ind_wet ==len(df_met_red.MS_Water): # chatching boundary value problem
                pass
            else:
                ...
                # if df_met_red.MS_Water.iloc[ind_wet] >= 0 * df_met_red.SWE.iloc[ind_wet]:
                    ...
                    if df_prof_wet.date[0] != dy:
                        print('date of season_list_red (profiles) -',df_prof_wet.date[0],'- does not match date of df_P (wet cycle)-',dy)
    ...
    #########################################################################       
    #%% check new snow and the prior surface....298
    #  we only enter if NO prior non-persistent problem pre-exists
    #thiscould also work for wind slabs covering WLs, if we use deltahs instead of hn24
    #  but snowpack flatfield sim do not in-/decrease snow height when 24h_wind_drift>0 !?          
                ...
                if index == 0: #catching the entrance problem with index0
                    ...
                    if idx > 0 and idx < (len(df_prof[po])-2): 
                        print('new problem?',index, df_P.dy[index])
                        # window to check out properties of old snow surface
                        # % get grain types "around" old surface, min fun prevents so exceed length of array
                        selgt_all = df_prof.graintype[idx-1:idx+4] # mabe implement chase where less than idx+3
                        selgt = []
                        for item in selgt_all:
                            selgt.append(item[0])
                        selgt = pd.Series(selgt)
                        ...
                        if sum(id_p.values)> 0:  # matlab 319       
                            ##%% new peristent WL (buried)
                            #% if persistent WL exists already, make room for new pap
                            #% so, we add the pap to the list of daps
                            if df_P.loc[index, 'papex'] == 1:
                                print(' pap --> dap', df_P.loc[index, 'dy'])
                            ...
                            print( 'new... pap... ', df_P.papgt[index],'down',[df_P.papSLdep[index]*100],'cm')
                            ...
                            if (df_P.loc[index, 'papSLdep'] > minSLthrsh and df_P.loc[index, 'papSLrho'] > minSLdens) \
                                ...
                                #  no slab --> label as persistent problem for future
                                print ('    no slab yet --> keep pap')
                                df_P.loc[index, 'papcalc'] = 0
                                # start evolving shear strength           
                                #df_P.papINI[index] = []
                ...
                ###
                            ...
                            ## new non-peristent WL (buried)
                            # position of WL == old snow surface
                    ...
                    if df_P.napcalc[index] != 1 and df_P.drft.iloc[index] > drftthrsh:
                        ...
                        df_P.loc[index,'napPRO_wf']  = PRO[2]  
                        df_P.loc[index,'napPRO_ac_ga17']  = PRO[3] 
                        print('!!  wind slabs possibly /wo new snow')

        runno = runno+1
    return df_met_red , df_P
#Visualisation

# %%
def find_lay_prof(df_prof, Lup,Lgt,wlopt):
        #% read extracted profile for a single day
        #% find the exact location of a layer
        #% find weak layer based on its class defined in wlopt: 'per' or 'nonper'
